Shader "Sonar/SonarObject"
{
    ...
    SubShader
    {
        ...
        // Le bruit deplace les sommets autour du CENTRE de l'objet
        // (reproduit TestShaderProps qui ecrit Position = noise * normal
        //  en object space, donc centre sur le pivot de l'objet)
        float3 NoiseDisplace(float3 posWS, float3 normWS)
        {
            // Centre world de l'objet (colonne 3 de la matrice model)
            float3 objCenter = float3(UNITY_MATRIX_M[0].w, UNITY_MATRIX_M[1].w, UNITY_MATRIX_M[2].w);

            float3 animated = posWS + (_NoiseSpeed * _Time.y);
            float  n = SimpleNoise(animated.xy, _NoiseScale);
            float  d = n * _NoiseStrength;

            // Deplace depuis le centre de l'objet le long de la normale
            return objCenter + normWS * d;
        }
